...
try:
    ...
    names = [d.name for d, _s, _x, _b in pads]
    check("Holdtest Pad One" in names, "the one that was there is picked up")
    check(not gone, "and nothing has left yet")
    watched = {d.path for d, _s, _x, _b in pads}
    check(all(s for _d, s, _x, _b in pads), "each has a Start to watch for")
    ...
    names2 = [d.name for d, _s, _x, _b in pads2]
    ...
    before = {d.path: id(d) for d, _s, _x, _b in pads}
    kept = [id(d) for d, _s, _x, _b in pads2 if d.path in before]
    ...
    check("Holdtest Pad Two" not in [d.name for d, _s, _x, _b in pads3],
